...
## Extra Features & Creative UI Choices
    ...
    Image Fade Mask: Destination images use a horizontal gradient mask to fade smoothly into transparency, creating a polished card layout.
    “View Logs” Button: A full‑width button at the top of the list screen provides quick access to saved logs. Its height and styling were customized for a sleek, modern look.
    Center‑Aligned Top Bar: The app bar title is centered using CenterAlignedTopAppBar for a balanced header design.
    Toast Notifications: Subtle feedback is given when notes are updated, when leaving the details screen, and when logs are saved.
